...
def start_wsa_alert_matrix_job(
    ...
    job = job_manager.create()
    job_manager.run_in_background(
        job, lambda active_job: get_account_wsa_alert_matrix(account_key, data, active_job, context)
    )
    return {'jobId': job.job_id}


@app.get(f'{API_PREFIX}/dashboard/account/{{account_key}}/wsaAlertMatrix/jobs/{{job_id}}/events')
def stream_wsa_alert_matrix_job(account_key: str, job_id: str) -> StreamingResponse:
    job = job_manager.get(job_id)
    if not job:
        raise HTTPException(status_code=404, detail='Job not found')
    return _job_event_stream(job)
...
def start_wsa_alert_matrix_summary_job(
    ...
    job = job_manager.create()
    job_manager.run_in_background(
        job, lambda active_job: get_account_wsa_alert_matrix_summary(account_key, data, active_job, context)
    )
    return {'jobId': job.job_id}


@app.get(f'{API_PREFIX}/dashboard/account/{{account_key}}/wsaAlertMatrix/summary/jobs/{{job_id}}/events')
def stream_wsa_alert_matrix_summary_job(account_key: str, job_id: str) -> StreamingResponse:
    job = job_manager.get(job_id)
    if not job:
        raise HTTPException(status_code=404, detail='Job not found')
    return _job_event_stream(job)
...
def wsa_alert_matrix_summary_json(
    ...
    """Synchronous summary endpoint (no job/SSE wrapper) for other components to consume directly."""
    try:
        return get_account_wsa_alert_matrix_summary(account_key, data, None, context)
    except Exception as error:
        raise HTTPException(status_code=500, detail=str(error))
...
def wsa_alert_matrix_scorecard_json(
    ...
    """Synchronous scoreCard endpoint: featureName/count/configs JSON, no job/SSE wrapper."""
    try:
