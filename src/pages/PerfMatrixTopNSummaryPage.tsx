...
import {
  startPerfMatrixTopNSummaryJob,
  subscribeToPerfMatrixTopNSummaryJob,
...
export function PerfMatrixTopNSummaryPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        const jobId = await startPerfMatrixTopNSummaryJob(accountId, dataMode, context || undefined)
        if (!isMounted) {
          return
        }

        unsubscribeRef.current = subscribeToPerfMatrixTopNSummaryJob(
          accountId,
          jobId,
