...
import {
  startSecHostCoverageMatrixSummaryJob,
  subscribeToSecHostCoverageMatrixSummaryJob,
...
export function SecHostCoverageMatrixSummaryPage() {
  ...
  const [result, setResult] = useState<SecHostCoverageMatrixSummaryResult | null>(null)
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        const jobId = await startSecHostCoverageMatrixSummaryJob(accountId, dataMode, context || undefined)
        if (!isMounted) {
          return
        }

        unsubscribeRef.current = subscribeToSecHostCoverageMatrixSummaryJob(
          accountId,
          jobId,
          (event: SecHostCoverageMatrixSummaryJobProgressEvent) => {
            if (!isMounted) {
              return
            }
  ...
  return (
    <DashboardLayout title="Security Host Coverage Matrix Summary">
      <div className="space-y-6">
        ...
        {result ? (
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-card">
            ...
            <div className="mb-6 flex flex-wrap gap-2 text-xs font-semibold text-slate-700">
              {Object.entries(result.metricTotals).map(([metric, total]) => (
                <span key={metric} className="rounded-full bg-sky-100 px-3 py-1 text-sky-700">
                  {metric} (Count): {total}
                </span>
              ))}
            </div>
            ...
            <div className="mb-4 flex flex-wrap items-center gap-3">
              <label className="flex items-center gap-2 text-sm font-semibold text-slate-700">
                ...
                >
                  <option value={OVERALL_COLUMN}>Overall Coverage (covered / not covered)</option>
