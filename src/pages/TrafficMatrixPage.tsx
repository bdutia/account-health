...
import { startTrafficMatrixJob, subscribeToTrafficMatrixJob } from '../services/trafficMatrixJobs'
import type {
  CsvDataMode,
  JobProgressLevel,
...
export function TrafficMatrixPage() {
  ...
  const [result, setResult] = useState<TrafficMatrixResult | null>(null)
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        const jobId = await startTrafficMatrixJob(accountId, dataMode, context || undefined)
        if (!isMounted) {
          return
        }

        unsubscribeRef.current = subscribeToTrafficMatrixJob(
          accountId,
          jobId,
          (event: TrafficMatrixJobProgressEvent) => {
            if (!isMounted) {
              return
            }
  ...
  // Deep-link support: /trafficMatrix/:hostname pre-selects a hostname.
  useEffect(() => {
  ...
  return (
    <DashboardLayout title="Traffic Matrix">
        ...
        {result ? (
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-card">
            <div className="mb-4 flex flex-wrap items-center gap-3">
              <h2 className="text-lg font-bold text-slate-800">Hostname Traffic Matrix</h2>
