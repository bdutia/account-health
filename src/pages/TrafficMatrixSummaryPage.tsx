...
import {
  startTrafficMatrixSummaryJob,
  subscribeToTrafficMatrixSummaryJob,
...
export function TrafficMatrixSummaryPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        const jobId = await startTrafficMatrixSummaryJob(accountId, dataMode, context || undefined)
        if (!isMounted) {
          return
        }

        unsubscribeRef.current = subscribeToTrafficMatrixSummaryJob(
          accountId,
          jobId,
